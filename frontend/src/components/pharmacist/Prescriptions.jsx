...
const Prescriptions = () => {
    ...
    const filteredCompletedPrescriptions = completedPrescriptions.filter(prescription =>
        prescription.id.toLowerCase().includes(searchQuery.toLowerCase()) ||
        prescription.patient.toLowerCase().includes(searchQuery.toLowerCase()) ||
        prescription.doctor.toLowerCase().includes(searchQuery.toLowerCase()) ||
        prescription.medicines.some(medicine => 
    ...
    return (
        <div className="p-8 bg-gray-50 dark:bg-gray-950 min-h-screen font-sans antialiased">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 mt-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100">Completed Prescriptions ({filteredCompletedPrescriptions.length})</h3>
                </div>
                <div className="max-h-[70vh] overflow-y-auto pr-2">
                    <ul className="space-y-4">
                        {filteredCompletedPrescriptions.length > 0 ? (
                            filteredCompletedPrescriptions.map((prescription) => (
                                <li 
                                    key={prescription.id} 
